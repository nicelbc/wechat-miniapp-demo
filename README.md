...
- ✅ 碰撞检测和方块旋转（支持墙踢）
- ✅ 行消除和计分系统（单行100分，双行300分，三行500分，Tetris 800分）
- ✅ 关卡进度（每10行升一级，最大10级，速度随关卡增加）
- ✅ 高分记录（本地存储前10名）
- ✅ 暂停/继续功能
- ✅ 移动端触摸控制（滑动、点击、长按）
- ✅ 下一个方块预览
- ✅ 游戏结束检测
- ✅ **视觉优化**：浅灰色主题，提高对比度和可读性
- ✅ **边界移动优化**：修复最右侧移动问题，确保所有方块都能到达有效位置
...
```json
{
  "pages": ["pages/index/index", "pages/logs/logs", "pages/tetris/tetris"],
  "window": {
...
#### 1. 颜色优化（2026-02-28）
- **问题**：游戏板和方块颜色过暗，视觉效果不佳
- **修复**：
  - 游戏板背景色：`#222` → `#f0f0f0`（浅灰色）
  - 空单元格背景色：`#111` → `#e8e8e8`（更浅的灰色）
  - 边框颜色：`#333` → `#bbb`（中等灰色）
- **效果**：显著提高视觉对比度，游戏体验更佳
...
- **原因**：渲染定位公式未考虑单元格边距
- **修复**：
  - 定位公式：`{{(currentX + colIndex) * 30}}rpx` → `{{(currentX + colIndex) * 32}}rpx`
  - 游戏板宽度：`304rpx` → `324rpx`
  - 单元格边距：`margin: 1rpx` → `margin: 0 1rpx`（仅水平方向）
- **效果**：所有方块都能正确到达最右侧有效位置：
  - I 方块（宽度4）：最右侧 X = 6（单元格：6,7,8,9）
  - O 方块（宽度2）：最右侧 X = 8（单元格：8,9）
  - T 方块（宽度3）：最右侧 X = 7（单元格：7,8,9）
  - S/Z/J/L 方块（宽度3）：最右侧 X = 7（单元格：7,8,9）

详细修复信息请查看 `BUGFIX_SUMMARY.md` 文件。
...
### pages/tetris/tetris.js - 俄罗斯方块游戏逻辑
- `initGame()`: 初始化游戏状态
- `spawnNewPiece()`: 生成新方块
- `checkCollision()`: 碰撞检测
- `rotatePiece()`: 方块旋转
...
- `pauseGame()`: 暂停/继续游戏
- `gameOver()`: 游戏结束处理
- `saveHighScore()`: 保存高分记录
- `handleTouchStart/Move/End()`: 触摸控制
